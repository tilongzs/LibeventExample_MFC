...
static void libws_rdcb(struct bufferevent *bev, void *ctx)
{
	...
	if (res)
	{
		...
		case WS_OP_CONTINUE:
			break;
		case WS_OP_PING:
			ws->Send(&buf[msg.headerSize], msg.dataSize, WS_OP_PONG);
			break;
		case WS_OP_PONG:
			break;
		case WS_OP_TEXT:
...
int LibeventWS::Send(uint8_t* pdata, size_t dataSize, uint8_t op)
{
    if(!evConn)   // 未连接成功
        return -4;
    if(!is_active) // 连接无效
	...
	uint8_t header[14] = {0};
	size_t headerSize = makeHeader(dataSize, op, is_client, header);
